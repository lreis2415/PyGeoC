...
# Example: Given value's raster connectivity analysis,
...
def pixels_connectivity_compute(raster, i, j, idx):
    ...
    for di in [-1, 0, 1]:
        for dj in [-1, 0, 1]:
            if (i + di >= 0 and i + di < nrows and j + dj >= 0 and j + dj <
                ncols):
                if (raster[i + di][j + dj] == value and not (di == dj and di
                    == 0)):
...
def remove_existID_pixels(givenvalue_pixels_positions, idx_array):
    ...
    givenvalue_pixels_positions = np.delete(givenvalue_pixels_positions, existID_pixels, 0)
...
def draw_ID(ID, idx_array, drawID_raster):
    ...
    for i in range (idx_array.shape[0]):
        x = idx_array[i,0]; y = idx_array[i,1]
        drawID_raster[x,y] = ID
    return drawID_raster


# Example: Compute the craters pixels' connectivity and draw an ID rasterfile.
inputRaster = "RFCalcTest1Craters-PostProcessing.tif"
RasterData = ReadRaster(inputRaster)
...
WriteGTiffFile("OldTest1CratersID.tif", RasterData.nRows, RasterData.nCols,
               CratersIDData, RasterData.geotrans, RasterData.srs, -9999,
               GDT_Float32)